...
namespace Db\Table
{
    ...
    class hives_data extends Database
    {
        ...
        public function GetAllBySensorID_6HourAvg(int $sensor_id, string $past_date_limit, $limit = null): array
        {
            $CustomQuery = "";
            $CustomQuery .= "SELECT CAST(AVG(`weight_gm`) AS INT), CAST(AVG(`volts_mv`) AS INT), DATE_FORMAT(`datetime`, '%Y-%m-%d %H:00:00') FROM `" . $this->GetTableName() . "`";
            $CustomQuery .= " WHERE (`hive_sensor_id` = :sensor_id AND `datetime` >= :past_limit)";
        ...
        {
            $CustomQuery = "";
            $CustomQuery .= "SELECT CAST(AVG(`weight_gm`) AS INT), CAST(AVG(`volts_mv`) AS INT), DATE_FORMAT(`datetime`, '%Y-%m-%d') FROM `" . $this->GetTableName() . "`";
            $CustomQuery .= " WHERE (`hive_sensor_id` = :sensor_id AND `datetime` >= :past_limit)";
            # Order by date
        ...
        {
            $CustomQuery = "";
            $CustomQuery .= "SELECT CAST(AVG(`weight_gm`) AS INT), CAST(AVG(`volts_mv`) AS INT), DATE_FORMAT(`datetime`, '%b %Y') FROM `" . $this->GetTableName() . "`";
            $CustomQuery .= " WHERE (`hive_sensor_id` = :sensor_id AND `datetime` >= :past_limit)";
            # Order by date
